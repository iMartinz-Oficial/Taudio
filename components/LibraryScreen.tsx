              ...
              <div key={doc.id} className="relative">
                ...
                {menuOpenId === doc.id && (
                  <div className="absolute right-4 top-14 w-48 bg-white dark:bg-surface-dark shadow-2xl rounded-2xl z-50 py-2 border border-black/5 dark:border-white/10">
                    <button onClick={() => startEdit(doc)} className="w-full px-4 py-3 text-left text-sm font-bold flex items-center gap-2 hover:bg-primary/10 hover:text-primary">
          ...
          <div className="w-full max-w-md bg-white dark:bg-surface-dark rounded-[40px] p-6 shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleAddSubmit} className="flex-1 overflow-y-auto no-scrollbar space-y-4 pr-1">
              <input 
                ...
                className="w-full bg-slate-100 dark:bg-slate-800 rounded-2xl px-4 py-3 font-bold outline-none"
                value={newTitle}
              ...
              <div className="space-y-3">
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 px-1">Elegir Voz</p>
                <div className="grid grid-cols-2 gap-2">
                  {AI_VOICES.map((v) => (
                    <button key={v.name} type="button" onClick={() => setSelectedVoice(v.name as VoiceName)} className={`flex items-center gap-3 px-3 py-3 rounded-xl border transition-all ${selectedVoice === v.name ? 'border-primary bg-primary/10 text-primary' : 'border-black/5 dark:border-white/5 bg-black/5 dark:bg-white/5'}`}>
